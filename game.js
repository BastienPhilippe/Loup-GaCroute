...
class Game {
    ...
    async startGame() {
        ...
        this.middleCards = [new MiddleCard("carte A"), new MiddleCard("carte B"), new MiddleCard("carte C")]

        this.citizens.push(...this.players, ...this.middleCards)

        this.messageVillage("La partie commence !!!!");
        this.pushLine();
    ...
    wakeUp() {
        ...
        this.morningNews.push("A vous de jouer pour trouver qui sont les loups garous")
    ...
    getVote() {
        return this.players.map(player => {
                ...
                return player.pickPlayer(this.players)
            })
    }
    ...
    execution(votes) {
        ...
        if(deadPlayers.length === 0) {
            ...
            this.messageVillage(`Vous avez décidé de tuer ${
                deadPlayers.map(player => player.getName()).join("et ")
            }`)
        }
        return deadPlayers
    }
    ...
    _morningDebug() {
        this.players.forEach(player => {
            console.log(`${player.getName()} : ${player.initialRole.name} => ${player.role.name} team : ${player.role.team}`)
        })
        console.log("**********************************************************")
        this.middleCards.forEach(card => {
            console.log(`${card.getName()} : ${card.initialRole.name} => ${card.role.name}`)
        })
    }

    whoWins(deadPlayers) {
        const unLoupEstMort = deadPlayers.some(player => player.role.team === "loup");
        const pasDeLoup = this.players.every(player => player.role.team !== "loup");
    ...
    finalRecap(winningTeam) {
        function displayPlayer(player, game) {
            game.messageVillage(`${player.getName()} : role initial ${player.initialRole.name}, role en fin de partie ${player.role.name}`)
        } 
        this.pushLine()
        ...
        this.players.filter(player => player.role.team === winningTeam).forEach(winningPlayer => {
        ...
        this.players.filter(player => player.role.team === losingTeam).forEach(loser => {
