...
class Role{
    ...
    async checkAutreLoup(game, player) {
        ...
        if(autreLoups.length === 0) {
            player.sendDM("Il n'y a pas d'autre loup garou, vous pouvez donc regarder une carte au centre")
            const pickedCard = await player.pickPlayer(game.middleCards);
            pickedCard.sendRole(player)
        } else {
            player.sendDM(`Les autres loup sont ${
                autreLoups.map(loup => loup.getName()).join(", ")
    ...
    sendIntro(player) {
        player.sendRole()
        player.sendDM(`**Vous faites partie de l'équipe ${this.team}**`)
        player.sendDM(`*${this.rule}*`)
    }
}
