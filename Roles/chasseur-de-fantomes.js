...
class ChasseurDeFantomes extends Role {
  constructor() {
    ...
    this.rule =
      "Le chasseur de fantôme peux regarder le role d'un autre joueur, mais attention si il découvre un loup garou, il change d'équipe et rejoin le camp des loups. Si ce n'est pas un loup garou, il peur prendre le risque de regarder le role d'un autre joueur, les mêmes règles s'appliquent";
  }
  ...
  async playerNightAction(game, player) {
    ...
    if (!transformation(pickedPlayer.team)) {
      ...
      const secondPickedPlayer = await player.pickAnotherPlayer(
        game.players,
        pickedPlayer
      );
